...
def get_last_input():
    ...
    print "[*] it been %d milliseconds since the last input event "%elapsed
...
def detect_sandbox():
    ...
    double_clicks_threshold = 0.250 #秒为单位
    ...
    while not detection_complete:
        ...
        if keypress_time is not None and previous_timestamp is not None:
            ...
            #计算两次点击相隔的时间
            elasped = keypress_time - previous_timestamp

            #间隔时间段的话，则为用户双击
            if elasped <= double_clicks_threshold:
                double_clicks += 1
